...
def run_benchmark_matrix(args: argparse.Namespace, logger: logging.Logger) -> List[ComponentResult]:
    """Run every (precision, resolution) cell requested on the CLI (all 4 by default)."""
    ...
    model_dirs = {"fp16": Path(args.fp16_model_dir), "int4": Path(args.int4_model_dir)}
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "measured as deltas from pipeline start point. Runs all 4 configurations "
            "(fp16/int4 x 512x512/1024x1024) by default."
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--device", default="GPU", help="OpenVINO device: GPU, CPU or AUTO.")
    parser.add_argument("--precisions", default="fp16,int4", help="Comma-separated precision tiers to run.")
    parser.add_argument(
        "--resolutions",
